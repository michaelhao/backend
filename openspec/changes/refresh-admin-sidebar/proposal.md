...
Claude Design 出的「店務管理儀表板」設計稿（方案 B 淺色版）確立淺色紫為品牌方向，但既有 [layouts/admin.blade.php](resources/views/layouts/admin.blade.php) sidebar 仍使用深色 `bg-gray-900 text-gray-100`，與設計稿配色斷裂。
...
- 修改 [resources/views/layouts/admin.blade.php](resources/views/layouts/admin.blade.php)：
    ...
    - Logo / 站名：紫色品牌色 `text-violet-600`
    - 導覽連結 active：`bg-gray-800 text-white` → `bg-violet-50 text-violet-600`（**不**改字重，維持 `font-medium`）
