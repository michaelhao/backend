...
## Purpose
後台商店管理（列表搜尋分頁、編輯更新、統一編號認證與儲存時伺服端重驗）的完整行為規格。威脅模型：內部後台、非公網暴露（brute-force 不在 scope）、能登入後台者即為受信任的內部人員；正式環境無對外網際網路連線（政府商工登記 API 為全站唯一外部呼叫，於正式環境必定失敗，列為已知限制）。合併自舊版 `spec/6-shops-system.md`、`spec/7-edit-route-id.md`（shops 部分）、`spec/8-maskstring-support.md`（Mask 遮蔽，helper 文件保留）與 2026-06-12 設計 review 修正（commit `615c876`、`40a5388`、`6c22dc4`、`21c9517`）。
