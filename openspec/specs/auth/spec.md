...
## Purpose
後台登入/登出、登入節流與密碼重設的完整行為規格。威脅模型：內部後台、非公網暴露（brute-force 不在 scope）；正式環境無對外網際網路連線（禁用外部 API）。合併自舊版 `spec/1-login.md`、`spec/14-login-security-review.md` 與 2026-06-11 設計 review 修正（commit `4adf510`、`9339bf4`）。
