# string-masking Specification

## Purpose
`App\Support\Mask` 顯示遮蔽工具的完整行為規格：`string()` 將字串奇數索引字元換 `*`（以字元為單位、多位元組安全）、`email()` 僅遮蔽 `@` 前的 local part。威脅模型：內部後台、非公網暴露（brute-force 不在 scope）、能登入後台者即為受信任之內部人員；正式環境無對外網際網路連線，本功能無任何外部呼叫。遮蔽定位為顯示層防肩窺、非機密控制。合併自舊版 `spec/8-maskstring-support.md`（過渡狀態，描述已被取代）、演進後現狀（commit `816ad29` 遷入靜態類別、`186da22` 移除全域 helper）與 2026-06-13 設計 review 修正（commit `b81e784` 補單元測試、`9f8083a` 雙端一致性註解）。Mask 在商店編輯頁的使用情境見 `openspec/specs/shop-management/spec.md`。
