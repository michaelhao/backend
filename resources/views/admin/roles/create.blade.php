...
@section('content')
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-gray-800">新增角色</h2>
    </div>

    <form method="POST" action="{{ route('roles.store') }}" class="bg-white rounded-lg shadow p-6 space-y-6">
        @csrf
        ...
        <div>
            ...
            <x-searchable-select name="default_route" :value="old('default_route', '')" :permissions="$permissions" />
            @error('default_route')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
